...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    def check_if_there_is_cell(cell)
    if (cell == nil)
       0
    else
       1
  ...
  def show_cells(array)
    ...
    while (column_index < x_cell_count)
      row_index = 0
      while (row_index < y_cell_count)
        puts "Cell x: #{column_index}, y: #{row_index} north: #{check_if_there_is_cell(array[column_index][row_index].north).to_s} south: #{check_if_there_is_cell(array[column_index][row_index].south).to_s} east: #{check_if_there_is_cell(array[column_index][row_index].east).to_s} west: #{check_if_there_is_cell(array[column_index][row_index].west).to_s}"
        row_index += 1
      end
      puts "-------- End of Column ---------"
      column_index += 1
